...
if [ "$args" ] 
  then
    ...
    for i in $(mdfind -onlyin "$dir" "$args"); do
        echo "<item arg=\"$i\" type=\"file\" uid=\"$i\"><title>`basename "$i"`</title><subtitle><![CDATA[$(xmlEncode $(head -1 "$i"))]]></subtitle><icon type=\"fileicon\">$i</icon></item>"
    done
    unset IFS
    ...
    for i in $(find "$dir" \! -name ".DS_Store" \! -name "Icon?" -maxdepth 2 -type f); do
        echo "<item arg=\"$i\" type=\"file\"><title>`basename "$i"`</title><subtitle><![CDATA[$(xmlEncode $(head -1 "$i"))]]></subtitle><icon type=\"fileicon\">$i</icon></item>"
    done
    unset IFS
